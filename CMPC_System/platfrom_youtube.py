...
# -- coding: utf-8 --
# @Time : 2024/12/25 09:58
# @Author : Genesis Ai
# @File : platfrom_youtube.py
...
"""打开 youtube 应用，进行youtube的一系列操作"""


def open_youtube(udid):
    try:
        print(f"[{udid}] 正在打开 Youtube...")
        ...
        # 过滤出com.google.android.youtube
        if 'com.google.android.youtube' in output:
            print(f"[{udid}] Youtube 已成功启动")
        else:
            print(f"[{udid}] Youtube 启动可能失败，尝试备用方法")
...
def close_youtube(udid):
    try:
        print(f"[{udid}] 正在关闭 Youtube...")
        # 强制停止 Youtube 应用
        subprocess.run([
            'adb', '-s', udid, 'shell',
            'am', 'force-stop', 'com.google.android.youtube'
        ...
        # 返回桌面
        back_to_home(udid)
        print(f"[{udid}] Youtube 已关闭并返回桌面")
        return True
    except Exception as e:
        print(f"[{udid}] 关闭 Youtube 失败: {e}")
        return False
